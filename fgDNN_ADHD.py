...
file_path = f'~/Fun_LassoNet/NonG/10_p100/ADHD_1.csv'
save_path = f'~/Fun_LassoNet/NonG/10_p100/results.csv'
df = pd.read_csv(file_path, index_col=0)

# Initialize an empty dictionary to store the split DataFrames
data_frames = {}

# Split the DataFrame into 116 smaller DataFrames with 7 columns each
for i in range(116):
    start_col = i * 7
    end_col = start_col + 7
